...
class MNISTNet(nn.Module):
    def __init__(self,
        ...
        alpha = 2
        self.layer1 = nn.Linear(in_features,alpha*100)
        self.layer2 = nn.Linear(alpha*100, alpha*100)
        self.layer3 = nn.Linear(alpha*100, alpha*100)
        self.layer4 = nn.Linear(alpha*100, alpha*100)
        self.layer5 = nn.Linear(alpha*100, out_features)
...
class DropoutTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.network = MNISTNet(in_features=28*28,out_features=10, 
                               dropout_p= 0.5, dropout_at_eval= True)
        self.train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=self.batch_size, shuffle=True, drop_last=True
            )
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=self.learning_rate) 
    ...
    def T_scaling(self, preds_list, benchmark):
        return torch.div(preds_list, benchmark)    
    ...
    def train(self):
        ...
        for _ in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):
                ...
                preds_list = []
                labels_list = []      
                benchmark = nn.Parameter(torch.ones(1).to(self.device))

                with torch.no_grad():
                    preds_list.append(pred)
                    labels_list.append(batch_y)

                preds_list = torch.cat(preds_list).to(self.device)
                labels_list = torch.cat(labels_list).to(self.device)


                loss = loss_fn(self.T_scaling(preds_list, benchmark), labels_list)
    ...
    def dropout_activation(self, model):
      for m in model.modules():
        if m.__class__.__name__.startswith('layer'):
            m.train()        
    ...
    def predict_probabilities(self, x: torch.Tensor, num_sample=100) -> torch.Tensor:
        ...
        for i in range(forward_passes):
            ...
            model.eval()
            self.dropout_activation(model)

            for i, x_sample in enumerate(x):
                x_sample = x_sample.to(self.device)  
                with torch.no_grad():
                    output_prob = F.softmax(model(x_sample), dim=0)
                    output_prob = output_prob.to(self.device)
                predictions = np.vstack((predictions, output_prob.cpu().numpy()))

            dropout_predictions = np.vstack((dropout_predictions,
                                            predictions[np.newaxis, :, :]))
        
        estimated_0 = np.mean(dropout_predictions, axis=0).astype(np.float32) 
        estimated_probability = torch.from_numpy(estimated_0).to(torch.float32)

...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    layers = []
    # Create the hidden layers of the MLP
    for i in range(len(sizes)-2):
        layers.append(nn.Linear(sizes[i], sizes[i+1]))
        layers.append(activation())
    # Create the output layer of the MLP
...
class VPGBuffer:
    ...
    def end_traj(self, last_val=0):
        ...
        val_difference = vals[1:] - vals[:-1]
        rewards_path = rews[:-1]

        self.tdres_buf[path_slice] = discount_cumsum(rewards_path + val_difference, self.gamma * self.lam)
     
        # TODO: Implement discounted rewards-to-go calculation. 
        # Hint: use the discount_cumsum function 
        # self.ret_buf[path_slice] = ...
        
        self.ret_buf[path_slice] = discount_cumsum(rewards_path, self.gamma)
...
class Agent:
    ...
    def step(self, state):
        ...
        with torch.no_grad():
          ...
          pi = self.actor._distribution(state.float())
          act = pi.sample()
          log_prob = self.actor._log_prob_from_distribution(pi,act.float())

        v = state_value
        logp = log_prob
        
        return act.item(), v.item(), logp
...
def train(env, seed=0):
    ...
    for epoch in range(epochs):
        ...
        pi, logp = agent.actor(obs, actions)

        logp_loss = (-(logp * actions))

        logp_loss.requires_grad_()
        td_residuals.requires_grad_()

        actor_loss = (logp_loss * td_residuals).mean()
        ...
        for _ in range(100):
            ...
            values_pred = agent.critic(obs)

            mse_loss = nn.MSELoss()            
            critic_loss = mse_loss(values_pred, returns)
